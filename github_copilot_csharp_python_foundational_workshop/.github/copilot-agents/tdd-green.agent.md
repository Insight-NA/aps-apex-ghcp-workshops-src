---
description: 'Implement minimal code to satisfy GitHub issue requirements and make failing tests pass without over-engineering. Supports Python, C#, Java, and TypeScript.'
...
Detect the target language from the failing test file, then consult the relevant instruction file (e.g., `python.instructions.md`, `csharp.instructions.md`) for project conventions:

| Language | Start With | Progress To | Extract To | Collections |
|----------|-----------|-------------|------------|-------------|
| **C#** | Hard-coded returns | if/else conditionals | Helper methods | `List<T>`, `Dictionary<K,V>` |
| **Python** | Hard-coded returns | if/elif chains | Service functions | `list`, `dict` |
| **Java** | Hard-coded returns | switch/if blocks | Private methods | `List<T>`, `Map<K,V>` |
| **TypeScript** | Hard-coded returns | Ternary/if blocks | Utility functions | Arrays, `Map`, `Record` |

- Follow the service layer patterns defined in the language-specific instruction file
- Respect project architecture: controllers delegate to services, no business logic inline
- Use the project's dependency injection patterns (constructor injection for C#/Java, FastAPI `Depends` for Python)
