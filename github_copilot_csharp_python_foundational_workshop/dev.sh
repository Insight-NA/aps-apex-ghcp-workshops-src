...
# =============================================================================
# Road Trip App - Development Environment Startup
# =============================================================================
#
# This script starts the full development environment with both frontends
# running simultaneously in Docker.
#
# Usage:
#   ./dev.sh          # Start all services
#   ./dev.sh build    # Rebuild all containers
...
print_banner() {
    echo -e "${BLUE}"
    echo "╔═══════════════════════════════════════════════════════════════╗"
    echo "║           🚗 Road Trip App - Development Environment          ║"
    echo "╚═══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
case "${1:-up}" in
    up|start)
        ...
        print_urls
        echo -e "${GREEN}Use './dev.sh logs' to view logs${NC}"
        ;;
    build)
        print_banner
        check_env
        ...
        echo -e "${GREEN}Build complete. Run './dev.sh' to start.${NC}"
        ;;
    down|stop)
