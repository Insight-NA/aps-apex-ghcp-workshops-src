...
> These prompts are designed for use with the [Playwright MCP server](https://playwright.dev/docs/mcp).
> Each prompt maps 1:1 to a test spec in `frontend/e2e/tests/mcp/`.
...
4. Inject a fake token into localStorage: execute JS to set `localStorage.setItem('token', 'test-token')` and `localStorage.setItem('user_email', 'test@example.com')`.
5. Reload the page.
...
2. Assert that four quick-start template cards are visible: "Weekend Getaway", "Cross Country", "National Parks", "Hidden Gems".
